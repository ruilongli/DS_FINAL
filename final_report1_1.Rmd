...
```{r}
CPI<-c(cpi$CPI,fake.testing.X[,67][[1]])
PPI<-c(ppi$PPI,fake.testing.X[,72][[1]])
inf_cpi<-diff(CPI,12)
inf_ppi<-diff(PPI,12)
train_index<-1:156
...
here in later part, varible with name pred_xx will record the predicted value of inflation and return the Out of sample R2 directly
```{r}
